...
Cen = [-1*sin(Pc(1,1))*cos(Pc(2,1)), -1*sin(Pc(1,1))*sin(Pc(2,1)), cos(Pc(1,1));
       -1*sin(Pc(2,1)), cos(Pc(2,1)), 0;
       -1*cos(Pc(1,1))*cos(Pc(2,1)), -1*cos(Pc(1,1))*sin(Pc(2,1)), -1*sin(Pc(1,1))];
Cen = Cen';
for i = 1:length(IMU_true)
    ...
    Vc = Vc + VDot*DeltaT;
    omega_en = [Vc(2,1)/(RE+Pc(3,1));...
        -1*Vc(1,1)/(RN+Pc(3,1));...
        -1*Vc(2,1)*SCLA/((RE+Pc(3,1))*CCLA)];
    %% Calculating new position
    Omega_en = [0 -1*omega_en(3, 1) omega_en(2,1);
                omega_en(3, 1) 0 -1*omega_en(1,1);
                -1*omega_en(2,1) omega_en(1,1) 0];
    CenDot = Cen*Omega_en;
    Cen = Cen + CenDot*DeltaT;

    %% Position Update:
    PDot(1,1) = asin(-Cen(3, 3));
    PDot(2,1) = atan2(-Cen(3, 2), -Cen(3, 1));
    PDot(3,1) = -1 * Vc(3,1);
